...
from dense_correspondence.network.dense_correspondence_network import (
    DenseCorrespondenceNetwork,
)
from dense_correspondence.loss_functions.pixelwise_contrastive_loss import (
    PixelwiseContrastiveLoss,
)
from dense_correspondence.loss_functions.pixelwise_distributional_loss import (
    PixelwiseDistributionalLoss,
)
...
from dense_correspondence.correspondence_tools.correspondence_finder import (
    random_sample_from_masked_image,
)
...
class DenseCorrespondenceEvaluation(object):
    ...
    def load_network_from_config(self, name):
        ...
        path_to_network_params = self._config["networks"][name][
            "path_to_network_params"
        ]
        ...
        dcn = DenseCorrespondenceNetwork.from_model_folder(
    ...
    def evaluate_network(
        dcn, dataset, num_image_pairs=25, num_matches_per_image_pair=100
    ):
        """
        ...
        for i in range(0, num_image_pairs):
            ...
            if i % logging_rate == 0:
                print(
                    "computing statistics for image %d of %d, scene_name %s"
            ...
            dataframe_list_temp = (
                DCE.single_same_scene_image_pair_quantitative_analysis(
                    dcn,
                    dataset,
                    scene_name,
                    img_idx_a,
    ...
    def plot_descriptor_colormaps(
        ...
        mask_b=None,
        plot_masked=False,
        descriptor_norm_type="mask_image",
    ):
        """
        ...
        if plot_masked:
            ...
            if descriptor_image_stats is None:
                (
                    res_a_norm_mask,
                    res_b_norm_mask,
                ) = dc_plotting.normalize_masked_descriptor_pair(
    ...
    @staticmethod
    def single_same_scene_image_pair_quantitative_analysis(
        dcn, dataset, scene_name, img_a_idx, img_b_idx, num_matches=100, debug=False
    ):
        """
        ...
        (uv_a_vec, uv_b_vec) = correspondence_finder.batch_find_pixel_correspondences(
            img_a_knots, img_b_knots, device="GPU"
        )
        ...
        for i in match_list:
            ...
            pd_template = DCE.compute_descriptor_match_statistics(
                mask_a,
                mask_b,
                uv_a,
                uv_b,
    ...
    @staticmethod
    def compute_descriptor_match_statistics(
        mask_a,
        mask_b,
        uv_a,
        uv_b,
        ...
        (
            ...
            best_match_diff,
            norm_diffs,
        ) = DenseCorrespondenceNetwork.find_best_match(uv_a, res_a, res_b, debug=debug)
        ...
        pixel_match_error_l2 = min(
            np.linalg.norm((np.array(uv_b) - np.array(uv_b_pred)), ord=2),
            np.linalg.norm((np.array(uv_b_sym) - np.array(uv_b_pred)), ord=2),
        )
        pixel_match_error_l2_masked = min(
            np.linalg.norm((np.array(uv_b) - np.array(uv_b_pred_masked)), ord=2),
            np.linalg.norm((np.array(uv_b_sym) - np.array(uv_b_pred_masked)), ord=2),
        )
        pixel_match_error_l1 = min(
            np.linalg.norm((np.array(uv_b) - np.array(uv_b_pred)), ord=1),
        ...
        # compute this
        (
            v_indices_better_than_ground_truth,
            u_indices_better_than_ground_truth,
        ) = np.where(norm_diffs < norm_diff_descriptor_ground_truth)
        ...
        fraction_pixels_closer_than_ground_truth = (
            num_pixels_closer_than_ground_truth * 1.0 / num_pixels_in_image
        )

        (
            v_indices_better_than_ground_truth_masked,
            u_indices_better_than_ground_truth_masked,
        ) = np.where(masked_norm_diffs < norm_diff_descriptor_ground_truth)
        num_pixels_closer_than_ground_truth_masked = len(
            u_indices_better_than_ground_truth_masked
        )
        num_pixels_in_masked_image = len(np.nonzero(mask_b)[0])
        fraction_pixels_closer_than_ground_truth_masked = (
            num_pixels_closer_than_ground_truth_masked
            * 1.0
            / num_pixels_in_masked_image
        )
        ...
        if num_pixels_closer_than_ground_truth == 0:
            ...
            l2_distances = np.sqrt(
                (u_indices_better_than_ground_truth - uv_b[0]) ** 2
        ...
        if num_pixels_closer_than_ground_truth_masked == 0:
            ...
            l2_distances_masked = np.sqrt(
                (u_indices_better_than_ground_truth_masked - uv_b[0]) ** 2
        ...
        if debug:
            fig, axes = correspondence_plotter.plot_correspondences_direct(
                rgb_a, rgb_b, uv_a, uv_b, show=False
            )

            correspondence_plotter.plot_correspondences_direct(
                rgb_a,
                rgb_b,
                uv_a,
                uv_b_pred,
        ...
        pd_template.set_value(
            "pixel_match_error_l2_masked", pixel_match_error_l2_masked
        )
    ...
    def compute_loss_on_dataset(
        ...
        pixelwise_contrastive_loss = PixelwiseContrastiveLoss(
            dcn.image_shape, config=loss_config
        )
        ...
        for i, data in enumerate(data_loader, 0):
            ...
            if data_type == "matches":
                ...
                non_matches_a = Variable(
                    non_matches_a.cuda().squeeze(0), requires_grad=False
                )
                non_matches_b = Variable(
                    non_matches_b.cuda().squeeze(0), requires_grad=False
                )
            ...
            # get loss
            if data_type == "matches":
                loss, match_loss, non_match_loss = pixelwise_contrastive_loss.get_loss(
                    image_a_pred,
                    image_b_pred,
                    matches_a,
                    matches_b,
    ...
    @staticmethod
    def compute_descriptor_statistics_on_dataset(
        dcn, dataset, num_images=100, save_to_file=True, filename=None
    ):
        """
        ...
        if save_to_file:
            if filename is None:
                ...
                path_to_params_folder = utils.convert_to_absolute_path(
    ...
    def run_evaluation_on_network(
        ...
        if compute_descriptor_statistics:
            ...
            DCE.compute_descriptor_statistics_on_dataset(
        ...
        pd_dataframe_list, df = DCE.evaluate_network(
            dcn,
            dataset,
        ...
        fig_axes = DCEP.run_on_single_dataframe(
        ...
        if dataset.get_number_of_unique_single_objects() > 1:
            ...
            DCEP.run_on_single_dataframe_across_objects(
    ...
    @staticmethod
    def evaluate_network_by_zones(
        dcn, dataset, num_images=100, use_symmetry=False, num_matches=2000, sigma=3
    ):
        dcn.eval()
        ...
        for i, img_idx in enumerate(img_idxs):
            print(
                "Computed statistics on {}/{} images".format(i, len(img_idxs)), end="\r"
            )
            ...
            (
                uv_img_vec,
                uv_ref_vec,
            ) = correspondence_finder.batch_find_pixel_correspondences(
            ...
            for j in match_list:
                ...
                uv_ref_pred, _, _ = DenseCorrespondenceNetwork.find_best_match(
                    uv_img, res_img, res_ref, mask_b=mask_img
                )

                pixel_match_error_l2 = min(
                    np.linalg.norm((np.array(uv_ref) - np.array(uv_ref_pred)), ord=2),
                    np.linalg.norm(
                        (np.array(uv_ref_sym) - np.array(uv_ref_pred)), ord=2
                    ),
                )
    ...
    @staticmethod
    def evaluate_network_qualitative(
        dcn,
        dataset,
        type_of_analysis="single_match",
        num_pairs=5
    ):
        """Qualitative analysis of the network
        :param dcn: DenseCorrespondenceNetwork
        :param dataset: dataset
        :param type_of_analysis: ['general', 'single_match', 'circular', 'iterative', 'inverse_prediction']
        :param num_pairs: number of pairs of image to sample
        """
        dcn.eval()
        (
            scene_names,
            img_pairs,
        ) = DenseCorrespondenceEvaluation.get_random_scenes_and_image_pairs(
            dataset, num_pairs
        )
        for scene_name, img_pair in zip(scene_names, img_pairs):
            print("Image pair (%s, %s)" % (img_pair[0], img_pair[1]))
            data = DenseCorrespondenceEvaluation.get_data(
                dcn, dataset, scene_name, img_pair[0], img_pair[1]
            )
            if type_of_analysis == "single_match":
                DenseCorrespondenceEvaluation.single_match_qualitative_analysis(
                    data, pca_plot=True
                )
    ...
    def draw_a_circle(img_array, pixel_center, radius=14, color=COLOR_BLUE):
        if pixel_center is not None:
            img_array = cv2.circle(
                img_array, pixel_center, radius=radius, color=COLOR_WHITE, thickness=-1
            )
            img_array = cv2.circle(
                img_array, pixel_center, radius=radius - 4, color=color, thickness=-1
            )
        return img_array

    @staticmethod
    def get_data(
        dcn, dataset, scene_name, img_a_idx, img_b_idx, real_data=False, inverse=False
    ):
    ...
    def image_pair_qualitative_analysis(dcn, data, num_matches=7):
        ...
        for i in range(0, num_matches):
            ...
            best_match_uv, _, _ = DenseCorrespondenceNetwork.find_best_match(
                pixel_a, res_a, res_b
            )
            ...
            kp2.append(
                cv2.KeyPoint(float(best_match_uv[0]), float(best_match_uv[1]), diam)
            )
        ...
        if dcn.descriptor_dimension in [1, 3]:
            DenseCorrespondenceEvaluation.plot_descriptor_colormaps(
                res_a,
                res_b,
    ...
    def single_match_qualitative_analysis(data, pca_plot=False):
        ...
        rgb_a_array = DenseCorrespondenceEvaluation.draw_a_circle(
            rgb_a_array, pixel_a, color=COLOR_GREEN
        )
        axes[0, 0].imshow(rgb_a_array)
        axes[0, 0].set_title("Img A: Queried pixel")

        uv_b, _, norm_diffs = DenseCorrespondenceNetwork.find_best_match(
        ...
        rgb_b_array = DenseCorrespondenceEvaluation.draw_a_circle(
            rgb_b_array, uv_b, color=COLOR_RED
        )
        rgb_b_array = DenseCorrespondenceEvaluation.draw_a_circle(
            rgb_b_array, uv_b_masked, color=COLOR_BLUE
        )
        rgb_b_array = DenseCorrespondenceEvaluation.draw_a_circle(
            rgb_b_array, pixel_b, color=COLOR_GREEN
        )
        ...
        if loss_type == "distributional":
            ...
            norm_diffs = (norm_diffs - np.min(norm_diffs)) / (
                np.max(norm_diffs) - np.min(norm_diffs)
            )
        ...
        if pca_plot:
            ...
            res_b_pc = pca.fit_transform(res_b.reshape((-1, d))).reshape(
                res_b.shape[0], res_b.shape[1], -1
            )
            res_a_pc = pca.transform(res_a.reshape((-1, d))).reshape(
                res_a.shape[0], res_a.shape[1], -1
            )
    ...
    def circular_qualitative_analysis(data):
        ...
        for pixel_a_random in pixels_around_a:
            rgb_a_array = DenseCorrespondenceEvaluation.draw_a_circle(
                rgb_a_array, pixel_a_random, color=COLOR_RED, radius=5
            )
        ...
        for pixel_a_random in pixels_around_a:
            uv_b_masked_random, _, _ = DenseCorrespondenceNetwork.find_best_match(
                pixel_a_random, res_a.numpy(), res_b.numpy(), mask_b=mask_b
            )
            rgb_b_array = DenseCorrespondenceEvaluation.draw_a_circle(
                rgb_b_array, uv_b_masked_random, color=COLOR_RED, radius=5
            )
    ...
    def iterative_qualitative_analysis(data, num_iters=8):
        ...
        rgb_b_array = DenseCorrespondenceEvaluation.draw_a_circle(
            rgb_b_array, pixel_b, color=COLOR_BLUE
        )

        axes[0, 0].imshow(
            DenseCorrespondenceEvaluation.draw_a_circle(
                rgb_a_array, pixel_a, color=COLOR_BLUE
            )
        )
        axes[0, 0].set_title("Queried pixel")

        for iter in range(1, num_iters):
            uv_b, _, _ = DenseCorrespondenceNetwork.find_best_match(
                pixel_a, res_a.numpy(), res_b.numpy(), mask_b=mask_b
            )

            axes[iter // ncols, iter % ncols].imshow(
                DenseCorrespondenceEvaluation.draw_a_circle(
                    rgb_b_array, uv_b, color=COLOR_RED
                )
            )
    ...
    def inverse_prediction_analysis(data):
        ...
        closest_pixel_direct, _, _ = DenseCorrespondenceNetwork.find_best_match(
            pixel_b, res_b.numpy(), res_a.numpy(), mask_b=mask_a
        )
        rgb_a_array = DenseCorrespondenceEvaluation.draw_a_circle(
            rgb_a_array, closest_pixel_direct, color=COLOR_BLUE
        )
    ...
    def evaluate_match_quality(
        dcn, dataset, num_image_pairs=100, logging_rate=5, kl_loss=False
    ):
        """Evaluation of the quality of the matches as a classification problem"""
        DCE = DenseCorrespondenceEvaluation
        match_distances = []
        match_probabilities = []
        ...
        for i in range(0, num_image_pairs):
            ...
            idx_pair = DCE.get_image_pair_random(dataset, scene_name)
            img_idx_a, img_idx_b = idx_pair
            DCE.single_image_match_quality(
                dcn,
                dataset,
                scene_name,
                img_idx_a,
        ...
        cm_display = ConfusionMatrixDisplay(
            confusion_matrix=cf_matrix, display_labels=["Non match", "Match"]
        )
        cm_display.plot(ax=ax[1], cmap="inferno")
        plt.tight_layout()

    @staticmethod
    def single_image_match_quality(
        dcn,
        dataset,
        scene_name,
        img_a_idx,
        ...
        for i in match_list:
            ...
            # compute best match
            uv_b_pred, _, norm_diffs = DenseCorrespondenceNetwork.find_best_match(
                uv_a, res_a, res_b, mask_b=mask_b
            )
            ...
            if not kl_loss:
                ...
                radius = 2
                match_prob = 0
                for u_i, v_i in itertools.product(
                    range(u - radius, u + radius + 1), range(v - radius, v + radius + 1)
                ):
                ...
                masked_indices = pdl.flattened_mask_indices(
                    torch.tensor(mask_b), inverse=True
                )
                q_b = pdl.gauss_2d_distribution(
                    image_width, image_height, 1, u, v, masked_indices=masked_indices
                )
                loss = F.kl_div(
                    torch.tensor(p_b.ravel()).cuda().log(),
                    q_b,
                    reduction="sum",
                    log_target=False,
                )
    ...
    def evaluate_distribution(dcn, dataset, masking=False, pca_components=3):
        ...
        if pca_components == 2:
            ...
            for z in df["Shirt zone"].unique():
                ...
                ax[1].scatter(
                    x=df_aux["pc1"], y=df_aux["pc2"], c=df_aux["color"], s=2, label=z
                )
            ax[1].set_xlabel(
                "PC1 ({}%)".format(round(pca.explained_variance_ratio_[0], 2))
            )
            ax[1].set_ylabel(
                "PC2 ({}%)".format(round(pca.explained_variance_ratio_[1], 2))
            )
            ax[1].legend(
                bbox_to_anchor=(1.05, 1.0), loc="upper left", title="Shirt zone"
            )
            ...
            fig = px.scatter_3d(
                ...
                labels={
                    ...
                    "pc3": "PC3 ({}%)".format(
                        round(pca.explained_variance_ratio_[2], 2)
                    ),
                },
            )
...
class DenseCorrespondenceEvaluationPlotter(object):
    ...
    def make_pixel_match_error_plot(ax, df, label=None, num_bins=100, masked=False):
        ...
        plot = DCEP.make_cdf_plot(
            ax,
            data,
            num_bins=num_bins,
            label=label,
            x_axis_scale_factor=x_axis_scale_factor,
        )
        if masked:
    ...
    @staticmethod
    def make_norm_diff_ground_truth_plot(
        ax, df, label=None, num_bins=100, masked=False
    ):
        """
        :param ax: axis of a matplotlib plot to plot on
        :param df:
    ...
    @staticmethod
    def make_fraction_false_positives_plot(
        ax, df, label=None, num_bins=100, masked=False
    ):
        """
        :param ax: axis of a matplotlib plot to plot on
    ...
    @staticmethod
    def make_average_l2_false_positives_plot(
        ax, df, label=None, num_bins=100, masked=False
    ):
        """
        :param ax: axis of a matplotlib plot to plot on
    ...
    @staticmethod
    def run_on_single_dataframe(
        path_to_df_csv, label=None, output_dir=None, save=True, previous_fig_axes=None
    ):
        """
        ...
        axes[0].legend()
        if use_masked_plots:
            plot = DCEP.make_pixel_match_error_plot(
                axes[1], df, label=label, masked=True
            )
            axes[1].legend()
        return [fig, axes]

    @staticmethod
    def run_on_single_dataframe_across_objects(
        path_to_df_csv, label=None, output_dir=None, save=True, previous_fig_axes=None
    ):
        """
        This method is intended to be called from an ipython notebook for plotting.
