...
def normalize_vec(vec, stats=None):
    """
    Normalizes a vector to plot it in RGB space
    """
    vec = np.array(vec)
    if stats is None:
        vec_min = vec.min()
        ...
        vec_min = np.array(stats['min'])
        vec_max = np.array(stats['max'])

    normed_res = np.clip(vec, vec_min, vec_max)
    eps = 1e-10
    scale = (vec_max - vec_min) + eps
    normed_res = (normed_res - vec_min) / scale
    return normed_res
...
def pil_image_to_cv2(pil_image):
    ...
    return np.array(pil_image)[:, :,
           ::-1].copy()  # open and convert between BGR and RGB
